...
class SerialIO implements IIOProvider {
    ...
    public SerialIO(SerialPort.Port port_id, byte update_rate_hz, boolean processed_data,
    ...
    protected SerialPort resetSerialPort() {
        if (serial_port != null) {
            ...
            System.out.println(
                    "Closing " + (is_usb ? "USB " : "TTL UART ") + " serial port to communicate with navX-MXP/Micro");
            try {
                serial_port.close();
            } catch (Exception ex) {
    ...
    protected SerialPort getMaybeCreateSerialPort() {
        if (serial_port == null) {
            try {
                System.out.println("Opening " + (is_usb ? "USB " : "TTL UART ")
                        + " serial port to communicate with navX-MXP/Micro");
                ...
                System.out.println("Error opening " + (is_usb ? "USB " : "TTL UART ")
                        + " serial port to communicate with navX-MXP/Micro");
                serial_port = null;
            }
        }
    ...
    protected int decodePacketHandler(byte[] received_data, int offset, int bytes_remaining) {
        ...
        if ((packet_length = IMUProtocol.decodeYPRUpdate(received_data, offset, bytes_remaining,
                ypr_update_data)) > 0) {
            notify_sink.setYawPitchRoll(ypr_update_data, sensor_timestamp);
        } else if ((packet_length = AHRSProtocol.decodeAHRSPosTSUpdate(received_data, offset, bytes_remaining,
                ahrspos_ts_update_data)) > 0) {
            notify_sink.setAHRSPosData(ahrspos_ts_update_data, ahrspos_ts_update_data.timestamp);
        } else if ((packet_length = AHRSProtocol.decodeAHRSPosUpdate(received_data, offset, bytes_remaining,
                ahrspos_update_data)) > 0) {
            notify_sink.setAHRSPosData(ahrspos_update_data, sensor_timestamp);
        } else if ((packet_length = AHRSProtocol.decodeAHRSUpdate(received_data, offset, bytes_remaining,
                ahrs_update_data)) > 0) {
            notify_sink.setAHRSData(ahrs_update_data, sensor_timestamp);
        } else if ((packet_length = IMUProtocol.decodeGyroUpdate(received_data, offset, bytes_remaining,
                gyro_update_data)) > 0) {
            notify_sink.setRawData(gyro_update_data, sensor_timestamp);
        } else if ((packet_length = AHRSProtocol.decodeBoardIDGetResponse(received_data, offset, bytes_remaining,
                board_id)) > 0) {
        ...
        while (!boardid_request_sent) {
            try {
                ...
                cmd_packet_length = AHRSProtocol.encodeDataGetRequest(stream_command,
                        AHRSProtocol.AHRS_DATA_TYPE.BOARD_IDENTITY, (byte) 0);
                serial_port.write(stream_command, cmd_packet_length);
                serial_port.flush();
                port_reset_count++;
                if (debug) {
                ...
                // ex.printStackTrace();
                if (serial_port == null) {
                    System.out.println("Unable to open " + (is_usb ? "USB " : "TTL UART ")
                            + " serial port to communicate with navX-MXP/Micro");
                } else {
                    System.out.println("Error sending navX-MXP/Micro configuration request over "
                            + (is_usb ? "USB " : "TTL UART ") + " serial port.");
                    ex.printStackTrace();
                }
                Timer.delay(1.0);
                resetSerialPort();
        ...
        while (!stop) {
            try {
                ...
                if (serial_port == null) {
                    ...
                    if (debug)
                        System.out.println("Initiating reset of serial port, as serial_port reference is null.");
                    resetSerialPort();
                    continue;
                }
                ...
                if (signal_transmit_integration_control) {
                    ...
                    cmd_packet_length = AHRSProtocol.encodeIntegrationControlCmd(integration_control_command,
                            integration_control);
                    try {
                ...
                if (bytes_read > 0) {
                    ...
                    while (i < bytes_read) {
                            ...
                            if ((bytes_remaining > 2) &&
                                ...
                                while (bytes_remaining < total_expected_binary_data_bytes) {
                                    ...
                                    if (c.length > 0) {
                                        ...
                                        if (debug) {
                                            SmartDashboard.putNumber("navX Partial Binary Packets",
                        ...
                        if (packet_length > 0) {
                            ...
                            if (packet_length > 0) {
                                ...
                                packet_length = AHRSProtocol.decodeIntegrationControlResponse(received_data, i,
                                        bytes_remaining,
                                ...
                                if (packet_length > 0) {
                                    ...
                                    if (debug) {
                                        SmartDashboard.putNumber("navX Integration Control Response Count",
                                                integration_response_receive_count);
                                    }
                                    i += packet_length;
                                    if ((integration_control.action
                                            & AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_YAW) != 0) {
                                    ...
                                    if (discard_remainder) {
                                        ...
                                        if (!next_packet_start_found) {
                                            ...
                                            if ((bytes_remaining > 2) &&
                                                    (received_data[i
                                                            + 1] == AHRSProtocol.BINARY_PACKET_INDICATOR_CHAR)) {
                                                /* Binary packet received; next byte is packet length-2 */
                    ...
                    if (retransmit_stream_config ||
                        ...
                        try {
                            ...
                            if (is_usb) {
                                if (debug)
                                    System.out.println("Resetting serial port via reset().");
                                try {
                                    serial_port.reset();
                                } catch (RuntimeException ex2) {
                                    /* Sometimes an unclean status exception occurs during reset(). */
                ...
                ex.printStackTrace();
                if (debug)
                    System.out.println("Initiating Serial Port Reset due to exception during Run() loop.");
                resetSerialPort();
            }
        }
